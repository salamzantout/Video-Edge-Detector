...
behavior Monitor(i_img_receiver ImgIn, i_time_receiver StartTime)
{
	...
	******************************************************************************/
	...
	void main(void)
	{
	   ...
	   int i, n;
	   sim_time t1, t2, t, t3=0, t_diff1, t_diff2, fps1, fps2;
	   sim_time_string buf1, buf2, buf3, buf4, buf5, buf6;
	   ...
	   for(i=0; i<IMG_NUM; i++)
	   {
	      ...
	      t = t2 - t1;
	      n = i % AVAIL_IMG;
	      t_diff1 = ((t2-t3)/(1 MILLI_SEC))/1000;
	      t_diff2 = ((t2-t3)/(1 MILLI_SEC))%1000;

	      fps1 = (1000000/((t2-t3)/(1 MILLI_SEC)))/1000;
	      fps2 = (1000000/((t2-t3)/(1 MILLI_SEC)))%1000;

	      printf("%9s: Monitor received frame%3d with%9s "TIME_BASE_UNIT" delay.\n",
			time2str(buf2, t2/TIME_BASE), n+1, time2str(buf1, t/TIME_BASE));

	      printf("%9s:%s.%s seconds after previous frame, %s.%s FPS.\n",
			time2str(buf2, t2/TIME_BASE), time2str(buf3,t_diff1), time2str(buf4,t_diff2), time2str(buf5,fps1), time2str(buf6,fps2));
